          ...
          if (!(is.null(missing_date_range_check(input$relationship_date_range,
            ...
            observeEvent(input$render, {
              output$report <- renderUI({
                ...
                aws.s3::put_object(bucket = "awp-cow-bonds", object = filename)
                
                aws.s3::get_object(bucket = "awp-cow-bonds", object = filename) %>%
                  writeBin(paste0("www/", filename))
                
                tags$iframe(style = "height:200px; width:100%", 
                            src = filename,
                            seamless="seamless",
                            scrolling = TRUE)
              })
